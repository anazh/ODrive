...
克拉克变换是一种将三相交流信号变换为两相信号的方法。
克拉克变换可以通过将三个相互垂直的相量（a、b、c）映射到一个二维平面上来实现。
...
std::tuple<float, float, float, bool> SVM(float alpha, float beta) {
    float tA, tB, tC;
    int Sextant; // Sextant指的是将空间矢量分解为六个相等的角度扇区的过程。

    if (beta >= 0.0f) {
        if (alpha >= 0.0f) {
            //quadrant I
            if (one_by_sqrt3 * beta > alpha) 
                Sextant = 2; //sextant v2-v3 ， 设置 Sextant 的值为2，表示所处于第二扇区（sextant v2-v3）
            else
...
// 现在距离 deadline 还有多少时间。如果 deadline 已经过去了，返回值为 0（除非 deadline 已经过去很久了）。
...
// 如果指定的系统时间（以毫秒为单位）在未来，则返回非零值；否则返回 0。
